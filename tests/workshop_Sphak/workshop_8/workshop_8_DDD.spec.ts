...
test.describe("Sample Test", () => {
  ...
  test.beforeEach(async ({ page }) => {
    ...
    await pageObject.open(
      "file:///C:/Projekte/Playwright/tests/workshop_8/index.html",
    );
  });
  ...
  for (const data of Object.values(testData)) {
    if (
      ...
      data.testName === "Test 2 - Negative Test"
    ) {
      test.skip(data.testName, async () => {
        ...
        expect(await pageObject.text(pageObject.displayIsStudent)).toBe(
          data.expectedIsStudent,
        );
      });
    }
  }
