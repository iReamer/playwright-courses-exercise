...
test('handling right/doubleclick',async({page})=>{
    ...
    const doubleClickButton = page.getByRole('button',{name: 'Double-Click Me To See Alert'});
    ...
    await page.getByText('Quit').click();    await page.waitForTimeout(3000);
})


test('handling shift click',async({page})=>{
    await page.goto('https://the-internet.herokuapp.com/shifting_content');
    const exampleOneButton = page.getByRole('link',{name: 'Example 1: Menu Element'});
